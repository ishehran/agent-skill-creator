...
5. Fail on console errors and failed required network calls.
6. Add state seam for diagnostics (`window.__TEST__.state()`).
7. Lock test environment (viewport, locale, timezone, seeded data).
...
For MCP-driven browser automation in Codex, apply the same principles.
